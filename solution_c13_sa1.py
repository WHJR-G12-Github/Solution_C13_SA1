...
while True:  
  ...
  if state=="play":
      ...
      if groundx<-450:
       groundx=0
      # Checking if 'pipe1.tpipe.x' equals 'bird1.bird.x'
      if pipe1.tpipe.x == bird1.bird.x:
       # Incrementing 'score' by 1
       score=score+1
      # Calling the 'move()" method using the 'pipe1' object 
      pipe1.move() 
  ...
  for event in pygame.event.get():
    ...
    if event.type==pygame.KEYDOWN:
        if event.key==pygame.K_SPACE and state=="play":
            bird1.moveup()  
  # Checking if the bird collides with 'tpipe' or with the 'bpipe' or if y-value of bird is greater than 600(game screen's height)
  if bird1.bird.colliderect(pipe1.bpipe) or bird1.bird.colliderect(pipe1.tpipe) or bird1.bird.y>=600:
      # Assigning the value of 'state' to "over"
      state="over"
